...
Untrusted input is a contract clause; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a contract clause
- **Closed type:** `type Decision = FlagClause(ClauseRisk) | ApproveClause` where `ClauseRisk` is closed
- **Sink / capability:** `grant flagClause confidence 70` → `privileged { flagClause(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/legal-review-agent/demo.sh
```
...
- `legal-review-agent_safe.wave` — the correct design.
- `legal-review-agent_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
